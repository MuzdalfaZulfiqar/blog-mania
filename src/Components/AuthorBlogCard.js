...
function AuthorBlogCard(props) {
  let imageUrl  = "";
  let imageBlock = "";
  let content = props.content.blocks;
  if(content!= undefined){

    imageBlock = content.find((block) => block.type === "image");
    if (imageBlock) {
      imageUrl = imageBlock.data.file.url;
      const caption = imageBlock.data.caption;
    }
  }
  ...
  return (
    <div
      className="card mb-3 col-md-12 cursor-pointer"
      style={{ maxWidth: "740px" }}
      // onClick={()=>{
      //   props.viewBlog(props.id)
      // }}
    >
      <div className="row g-0 cursor-pointer">
        <div className="col-md-8">
          <div className="card-body">
            <h5 className="card-title">{props.name}</h5>
            <p className="card-text fw-bold cardTitle">{props.title}</p>
            <p className="card-text">{props.briefDescription}</p>
            <p className="card-text d-flex gap-4 align-items-center">
              ...
              <i
                ...
                onClick={() => {
                  // console.log("hi")
                  handleBlogDelete(props.id);
                }}
              ></i>
            </p>
          </div>
        </div>
        <div className="col-md-4 blogImg"
        // style={{
        //     width:""
        // }}
        >
